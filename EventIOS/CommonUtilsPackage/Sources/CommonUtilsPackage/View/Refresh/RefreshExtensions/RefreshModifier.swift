    ...
    func body(content: Content) -> some View {
        return GeometryReader { proxy in
            content
                ...
                .backgroundPreferenceValue(Refresh.HeaderAnchorKey.self) { headerAnchorKey -> Color in
                    DispatchQueue.main.async { self.update(proxy: proxy, value: headerAnchorKey) }
                    return Color.clear
                }
                .backgroundPreferenceValue(Refresh.FooterAnchorKey.self) { footerAnchorKey -> Color in
                    DispatchQueue.main.async { self.update(proxy: proxy, value: footerAnchorKey) }
                    return Color.clear
                }
                .id(self.id)
        }
